...
%   Detailed explanation goes here
    ...
    handles.features = {'Firing rate',struct('callback',@calcFiringRate,'args',[]);...
				...
				'P1 amplitude',struct('callback',@calcP1Amplitude,'args',[]);...
				'P2 amplitude',struct('callback',@calcP2Amplitude,'args',[]);...
				'N amplitude',struct('callback',@calcNAmplitude,'args',[]);...
				'P1-P2 ratio',struct('callback',@calcP1overP2,'args',[]);...
				'Spike asymmetry',struct('callback',@calcAsymmetry,'args',[]);...
				'P1-N duration',struct('callback',@calcP1N,'args',[]);...
				'N-P2 duration',struct('callback',@calcNP2,'args',[]);...
				'Spike half-width',struct('callback',@calcHalfWidth,'args',[]);...
				'P1-P2 integral',struct('callback',@calcP1P2Area,'args',[]);...
				'P1-N integral',struct('callback',@calcP1NArea,'args',[]);...
				'N-P2 integral',struct('callback',@calcNP2Area,'args',[]);...
				'Spike integral asymmetry',struct('callback',@calcAreaAsymmetry,'args',[]);...
				'Spike propagation direction',struct('callback',@calcPropDirectionUpDown,'args',{{'Threshold (%)';30}});...
				'Spike propagation distance',struct('callback',@calcPropDistance,'args',{{'Threshold (%)';30}});...
				'Spike propagation time',struct('callback',@calcPropTime,'args',{{'Threshold (%)';30}});...
				'Spike propagation speed',struct('callback',@calcPropSpeed,'args',{{'Threshold (%)';30}});...
                'Spike 2D expanse - absolute',struct('callback',@calcSpikeExpanseAbsolute,'args',{{'Threshold (uV)';50}});...
                'Spike 2D expanse - N1',struct('callback',@calcSpikeExpanseNegative,'args',{{'Threshold (uV)';0}});...
                'Spike 2D expanse - P2',struct('callback',@calcSpikeExpansePositive,'args',{{'Threshold (uV)';0}});...
...
function inputHandler(hObject,~,~)
    ...
    if ~strcmp(handles.outputDir,'')
        ...
        startdir = '/home/debreceni/Projects/MScOnlab/Adam/Data/Matlab/';
    end
